...
-- dummy handler
local process = {
	get = function(agent, rq, header) local res = mtask.call(agent, "lua", "http", "get", rq, header)  return "200", res end,
	post = function(agent, rq, header, data) res = mtask.call(agent, "lua", "http", "post", rq, header, data)  return "200", res end,
}
...
function MSG.open(fd, msg)
	...
	mtask.send(watchdog, "lua", "http", "open", fd, msg)
end
...
function MSG.close(fd)
	close_fd(fd)
	mtask.send(watchdog, "lua", "http", "close", fd)
end

function MSG.error(fd, msg)
	close_fd(fd)
	mtask.send(watchdog, "lua", "http", "error", fd, msg)
end
