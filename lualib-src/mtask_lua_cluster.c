...
#define TEMP_LENGTH 0x10007 /*65543 0x10000 = 2 ^ 16  = 2 ^ 6 * 2 ^10 = 64 X 1024 = 65536*/
...
packreq_number(lua_State *L, int session, void * msg, size_t sz) {
    ...
    fill_header(L, buf, (int)sz+9, msg);
    buf[2] = 0;
...
packreq_string(lua_State *L, int session, void * msg, size_t sz) {
    ...
    uint8_t buf[TEMP_LENGTH];
    fill_header(L, buf, (int)(sz+5+namelen), msg);
...
lpackresponse(lua_State *L) {
    ...
    uint8_t buf[TEMP_LENGTH];
    fill_header(L, buf, (int)sz+5, msg);
