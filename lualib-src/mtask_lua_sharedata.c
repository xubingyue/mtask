...
#define VALUE_TYPE_REAL     1
#define VALUE_TYPE_STRING      2
#define VALUE_TYPE_BOOLEAN     3
#define VALUE_TYPE_TABLE       4
#define VALUE_TYPE_INTEGER     5
...
struct node {
    ...
    uint32_t key_hash;
    uint8_t  key_type;   /*key type must be integer or string*/
    uint8_t  value_type;   /*value type can be number/string/boolean/table*/
...
setvalue(struct context * ctx, lua_State *L, int index, struct node *n) {
    ...
    switch(vt) {
        case LUA_TNIL:
            n->value_type = VALUE_TYPE_NIL;
            break;
        case LUA_TNUMBER:
            if (lua_isinteger(L, index)) {
                n->v.d = lua_tointeger(L, index);
                n->value_type = VALUE_TYPE_INTEGER;
            } else {
                n->v.n = lua_tonumber(L, index);
                n->value_type = VALUE_TYPE_REAL;
            }
            break;
        case LUA_TSTRING: {
            size_t sz = 0;
            ...
            n->value_type = VALUE_TYPE_STRING;
            break;
        }
        case LUA_TBOOLEAN:
            n->v.boolean = lua_toboolean(L, index);
            n->value_type = VALUE_TYPE_BOOLEAN;
            break;
        case LUA_TTABLE: {
...
fillnocolliding(lua_State *L, struct context *ctx) {
    ...
    while (lua_next(L, 1) != 0) {
        ...
        if (!ishashkey(ctx, L, -2, &key, &keyhash, &keytype)) {
            ...
            struct node * n = &tbl->hash[keyhash % tbl->size_hash];
            if (n->value_type == VALUE_TYPE_NIL) {
                n->key = key;
                n->key_type = keytype;
                n->key_hash = keyhash;
                n->next = -1;
...
fillcolliding(lua_State *L, struct context *ctx) {
    ...
    while (lua_next(L, 1) != 0) {
        ...
        uint32_t keyhash;
        if (ishashkey(ctx, L, -2, &key, &keyhash, &keytype)) {
            struct node * mainpos = &tbl->hash[keyhash % tbl->size_hash];
            if (!(mainpos->key_type == keytype && mainpos->key == key)) {
                // the key has not insert
                struct node * n = NULL;
                for (i=emptyslot;i<sizehash;i++) {
                    if (tbl->hash[i].value_type == VALUE_TYPE_NIL) {
                        n = &tbl->hash[i];
                        break;
                    }
                }
...
convtable(lua_State *L) {
    ...
    if (sizearray) {
        tbl->array_type = (uint8_t *)malloc(sizearray * sizeof(uint8_t));
        if (tbl->array_type == NULL) {
            goto memerror;
        }
        for (i=0;i<sizearray;i++) {
            tbl->array_type[i] = VALUE_TYPE_NIL;
        }
        tbl->array = (union value *)malloc(sizearray * sizeof(union value));
        if (tbl->array == NULL) {
            goto memerror;
        }
        tbl->size_array = sizearray;
    }
    ...
    if (sizehash) {
        ...
        for (i=0;i<sizehash;i++) {
            tbl->hash[i].value_type = VALUE_TYPE_NIL;
            tbl->hash[i].nocolliding = 0;
        }
        tbl->size_hash = sizehash;
...
delete_tbl(struct table *tbl) {
    int i;
    for (i=0;i<tbl->size_array;i++) {
        if (tbl->array_type[i] == VALUE_TYPE_TABLE) {
            delete_tbl(tbl->array[i].tbl);
        }
    }
    for (i=0;i<tbl->size_hash;i++) {
        if (tbl->hash[i].value_type == VALUE_TYPE_TABLE) {
...
lookup_key(struct table *tbl, uint32_t keyhash, int key, int keytype, const char *str, size_t sz) {
    if (tbl->size_hash == 0)
        return NULL;
    struct node *n = &tbl->hash[keyhash % tbl->size_hash];
    if (keyhash != n->key_hash && n->nocolliding)
        return NULL;
    for (;;) {
        if (keyhash == n->key_hash) {
            if (n->key_type == KEY_TYPE_INTEGER) {
                if (keytype == KEY_TYPE_INTEGER && n->key == key) {
                    return n;
                }
            } else {
...
lindexconf(lua_State *L) {
    ...
    if (kt == LUA_TNUMBER) {
        ...
        if (key > 0 && key <= tbl->size_array) {
            --key;
            pushvalue(L, tbl->L, tbl->array_type[key], &tbl->array[key]);
            return 1;
        }
        keytype = KEY_TYPE_INTEGER;
        keyhash = (uint32_t)key;
    ...
    if (n) {
        pushvalue(L, tbl->L, n->value_type, &n->v);
        return 1;
    } else {
        return 0;
    }
...
lnextkey(lua_State *L) {
    ...
    if (lua_isnoneornil(L,2)) {
        if (tbl->size_array > 0) {
            int i;
            for (i=0;i<tbl->size_array;i++) {
                if (tbl->array_type[i] != VALUE_TYPE_NIL) {
...
llen(lua_State *L) {
    struct table *tbl = get_table(L,1);
    lua_pushinteger(L, tbl->size_array);
    return 1;
}
...
lhashlen(lua_State *L) {
    struct table *tbl = get_table(L,1);
    lua_pushinteger(L, tbl->size_hash);
    return 1;
}
