...
lpushbuffer(lua_State *L) {
    ...
    if (msg==NULL) {
        return luaL_error(L, "need message block at parm 3");
    }
...
check_sep(struct buffer_node *node,int from,const char *sep,int seplen) {
    for (;;) {
        int sz = node->sz  - from;
        if (sz >= seplen) {
            return memcmp(node->msg+from, sep, seplen);
        }
        if(sz >0) {
...
/*userdata send_buffer  table pool,nil for check  ,string sep*/
static int
...
lunpack(lua_State *L) {
    ...
    if (message->type == MTASK_SOCKET_TYPE_UDP) {
        int addrsz = 0;
        const char *addstring = mtask_socket_udp_address(message, &addrsz);
        if (addstring) {
            lua_pushlstring(L, addstring, addrsz);
            return 5;
        }
    }
    return 4;
