--多lua 用 sprotoloader的模块来共享 VM里都加载相同的sproto协议定义。

--其实现原理是在C模块中提供了16个全局的slot可以通过sprotoloader.register或sprotoloader.save
--在初始化时,加载需要的协议,并保存在这些slot里.通常我们只需要两个slot,一个用于保存客户端到服务器的协议组，
--另一个用于保存服务器到客户端的协议组。分别位于 slot 1 和 2 。
--这样，在每个 vm 内，都可以通过 sprotoloader.load 把协议加载到 vm 中。
--注意：这套 api 并非线程安全。所以必须自行保证在初始化完毕后再做 load 操作。
--（load 本身是线程安全的）。
